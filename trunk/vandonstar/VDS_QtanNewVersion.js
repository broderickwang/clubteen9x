...
function ktdlqtanvds()
{
if( (vdsA.search(/Điền tên bài hát vào đây!/i) != -1) ||(vdsA.length < 5) || (vdsA.search(' ')== -1) )
{$('#vds_tenck').css('background-color','#FF0000');$('#vds_kt_1').val('');}
else
{$('#vds_tenck').css('background-color','#74FF2E');$('#vds_kt_1').val('O');}

if( vdsB.search('/FLASH')== -1 || vdsB.indexOf('FLASH') > 5 || vdsB.search('http://')== -1 || (vdsB.search('zing.vn')== -1 && vdsB.search('nhaccuatui')== -1 && vdsB.search('nhac.vui')== -1 && vdsB.search('nhacso.net')== -1 ))
{$('#vds_flash').css('background-color','#FF0000');$('#vds_kt_2').val('');}
else
{$('#vds_flash').css('background-color','#74FF2E');$('#vds_kt_2').val('K');}

if( vdsC.length < 200 )
{$('#vds_nlck').css('background-color','#FBFF8F');}
else
{$('#vds_nlck').css('background-color','#74FF2E');}

if( vdsD.length < 3 )
{$('#vds_guiden').css('background-color','#FF0000');$('#vds_kt_3').val('');}
else
{$('#vds_guiden').css('background-color','#74FF2E');$('#vds_kt_3').val('E');}

if( (vdsE.length < 30) || (vdsE.search(' ')== -1) )
{$('#vds_ntdyt').css('background-color','#FF0000');$('#vds_kt_4').val('');}
else
{$('#vds_ntdyt').css('background-color','#74FF2E');$('#vds_kt_4').val('Y');}

if($('#vds_kt_1').val()+$('#vds_kt_2').val()+$('#vds_kt_3').val()+$('#vds_kt_4').val()=='OKEY')
{
ok_qtan();
}
else
...
$('#vds_qtan_noidung').val('[table class=qtvds][tr][td class=vv]'+vdsB+'[/td][/tr][/table][table class=tdvds][tr][td class=tdyt_1][b]Lời nhắn tới '+vdsD+': [/b][/td][/tr][tr][td class=tdyt_2][scroll][i]'+vdsE+'[/i][/scroll][/td][/tr][/table][table class=lhvds][tr][td class=lh][b][color=#ff0000]Lời ca khúc:[/color][/b][hr][i]'+vdsC+'[/i][hr][right][b][size=10]http://www.vandonstar.com[/size][/b][/right][/td][/tr][/table]');
