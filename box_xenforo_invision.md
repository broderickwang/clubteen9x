```
http://vn-answer.123.st/ kéo xuống box mà coi

cách làm ( ẩn rồi Cười lớn )


tạo 1 js in the home page vs nội dung
...
row1,row2{background-color:none;}
.forum-desc {
margin: 15px 1px;}
...
.ipbtable.index-box tr,.row2.icon+td+td+.row1,.row2.icon+td+td,.row2.icon{border-bottom:1px solid #DDD!important}
.row2.centered {margin-left: -350px;}
table.ipbtable td { padding:0px;}
table.index-box td { height:autopx;}
.row2, .row1 { background-color: none;}
.row1.centered ,.row2.centered {font-weight: bold;}
.last.post-info,.row2.icon+td+td+.row1{width:35%}
thead { display:none;}
table.index-box td {
height: auto;
}
.row1.centered {border-bottom:0 none!important}
