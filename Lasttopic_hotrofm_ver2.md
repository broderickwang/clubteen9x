![http://imageshack.us/a/img824/5577/alasst1.png](http://imageshack.us/a/img824/5577/alasst1.png)
![http://imageshack.us/a/img854/589/alasst2.png](http://imageshack.us/a/img854/589/alasst2.png)
• Thông tin :
Lasttopic sử dụng feed (rss) cập nhật : tổng quát , tác giả và chuyên mục
Lasttopic có ảnh người gửi cuối + tên hiện dưới dạng tooltip

Nếu có thông tin hay tính năng mới sẽ thông báo và cập nhật sau



I. Code
ACP --> Display --> Templates --> Quản Lí Tổng Thể --> index\_body >> Thêm đoạn code sau **{JAVASCRIPT}**
...
<a class="popupctrl" id="yui-gen1"><span class="topchon" id="ctop_title_2">TV tích cực nhất
...
<td class="blocksubhead" nowrap="nowrap" align="left">
<div id="ctop_select_2" href="#" rel="nofollow" class="popupmenu">
<a  class="popupctrl" id="yui-gen1"><span class="topchon" id="ctop_title_2">TV tạo nhiều chủ đề
...
ACP --> Display --> Templates --> Quản Lí Tổng Thể --> Portal
...
<span style="display:none">{POSTER.NB_TOPICS}
...
<font color="blue">{classical_row.recent_topic_row.L_TITLE}
...
<p>  Thời gian gửi :
<font color="#FF6600">{classical_row.recent_topic_row.S_POSTTIME}
...
<p> <font color="brown"> Người gửi cuối:  

Unknown end tag for &lt;/font&gt;

    <!-- BEGIN switch_poster --> {classical_row.recent_topic_row.switch_poster.S_POSTER}  <!-- END switch_poster -->
...
var x = $(a).find('item');
var y = $('.newtopic');
for (var n = 0, j = 0; n < x.length, j < y.length; n++, j++)
{$(y).eq(j).find('.infolast')
.html('<font color="green">Khái quát: 

Unknown end tag for &lt;/font&gt;

' + $(x).eq(n).find('description')
.text() + '<br><font color="red">Tác giả: 

Unknown end tag for &lt;/font&gt;

' + $(x).eq(n).find('guid')
.next().text() + '<br><font color="blue">Từ chuyên mục: 

Unknown end tag for &lt;/font&gt;

' + $(x).eq(n).find('category').text()
...
$('#changstats_latestposts .newtopic .username a#tacgia') .each(function () {
var userdata = $(this).attr('href');
var username =  $(this).text();
$(this).load(userdata + '#main-content #profile-advanced-right img:eq(0) ', function() {
this.className = 't_avatar';
$(this).children('img').before('<datatip>'+username+'

Unknown end tag for &lt;/datatip&gt;

');
});
$(this).hover(function(){
$(this).find('datatip').show();
},function(){
$(this).find('datatip').hide();
});
});
});
...
<span style="display:none">{POSTER.NB_POSTS}
...
<a href="javascript://" class="popupctrl" id="yui-gen3"><span class="topchon" id="ctop_title_4">TV gửi nhiều tuần
...
<td class="blocksubhead" style="height: 1px;" nowrap="nowrap">
<div id="ctop_select_4" href="#" rel="nofollow" class="popupmenu">
<a href="javascript://" class="popupctrl" id="yui-gen3"><span class="topchon" id="ctop_title_4">TV gửi nhiều tháng
...
Chỉnh widgets như hình dưới :
![http://i72.servimg.com/u/f72/17/98/73/13/untitl10.jpg](http://i72.servimg.com/u/f72/17/98/73/13/untitl10.jpg)

Vào ![http://illiweb.com/fa/admin/icones/editer.png](http://illiweb.com/fa/admin/icones/editer.png) của recent\_topics rồi chỉnh :
Number of recent topics : 15
Activate the topics scrolling : Không

Tiếp theo vào ACP --> Quản Lí Tổng Thể --> Messages and e-mails --> Cấu hình diễn đàn --> Topics --> Chủ đề trong 1 trang : 15