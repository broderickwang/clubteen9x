...
//msgbox
    var $msgbox = function(o) {
        ...
        if(typeof(o) == 'string'){ o = { content:{type:'text', content:o} } }   // 如�?�参数给出字符串， 则直�?�进行提示（text）
        opts = o || {};                                                         // 用�?�?�收参数
        
        opts.width             = o.width             || 360;                    // 提示框�?�宽度
        opts.height            = o.height            || 200,                    // 提示框�?�高度
        opts.autoClose         = o.autoClose         || 0;                      // 自�?�关闭�?�时间, 0则不�?自�?�关闭
        opts.title             = o.title             || '',                 // 提示框� �题
        opts.wrapperClass      = o.wrapperClass      || 'msgbox_wrapper';       // 提示框外框class
        opts.titleClass        = o.titleClass        || 'msgbox_title';         // 提示框� �题class
        opts.closeClass        = o.closeClass        || 'msgbox_close';         // 提示框关闭按钮class
        opts.titleWrapperClass = o.titleWrapperClass || 'msgbox_title_wrapper'; // 提示框� �题行class
        opts.mainClass         = o.mainClass         || 'msgbox_main';          // 内容框class
        opts.bgClass           = o.bgClass           || 'msgbox_bg';            // 背景层class
        opts.buttonClass       = o.buttonClass       || 'msgbox_button';        // 内容框中button�?�class
        opts.inputboxClass     = o.inputboxClass     || 'msgbox_inputbox';      // 内容框中input box�?�class
        ...
        opts.onClose           = o.onClose           || function(){};           // 关闭时执行�?�事件 
        ...
        opts.bgOpacity         = o.bgOpacity         || 0.6;                    // from 0 to 1  背景透�?度
        opts.onAjaxed          = o.onAjaxed          || function(){};           // ajax执行完之�?�?�事件 
        opts.onInputed         = o.onInputed         || function(){};           // 输入框关闭�?�?�事件
        opts.drag              = typeof o.drag != 'boolean' ? true : o.drag; // 默认允许拖拽
        opts.animation         = typeof o.animation != 'number' ? 1 : o.animation
        
        var returnValue = false;        // 返�?值, 用�?confirm和input
        var relTop      = 0;            // 提示框离窗口�?边�?�距离
        var relLeft     = 0;            // 提示框离窗口左边�?�距离, 用�?页面�?�?�时保持窗口不�?�
        ...
        var $titleWrapper = $('<div class=msgTips><p>Bạn chưa <span class=tip_highlight>Đăng kA� </span>hoặc <span class=tip_highlight>Đăng nhập</span></p><p>ChA o mừng đến LeLoiOnline. HA�y gia nhập cA�ng chA�ng tA�i</p></div><div class=tipBtns><a href=/register class=tipsReg><span>Register/</span>Đăng kA�</a><a href=/login class=tipsLogin><span>Login/</span>Đăng nhập</a></div><div class=timer></div>')
        ...
        function autoCloseMe(autoClose){            
            if( autoClose > 0 && isVisible() ){  // 防止人为关闭�?,计时器还在运行
                autoCloseStr = "BA i viết sẽ hiển thị sau " + autoClose + " giA�y nữa.";
        ...
        function flashTitle(opacity, times, interval, flag){ // 闪烁� �题(模拟windows)
            if( times > 0 ) {
                flag = !flag;
                op = flag ? opacity : 1;
            ...
            switch(ctt.type){
                case 'input':
                    ...
                    var $yesButton = $("<input type=button value=' OK '>")
                        ...
                        .after(" &nbsp; &nbsp; ")
                        .click(function(){
                            opts.onInputed($inputbox.val());  // 返�?输入�?�值
                            closeMe();
                        });
